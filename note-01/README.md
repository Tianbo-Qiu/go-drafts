...
| `cmd/32-select-multiplex` | **`select` 最小用法**：合并两条不同节奏的源 channel |
| `cmd/33-select-default-nonblock` | **`default` 非阻塞**；演示 `return` / `done` / labeled break 三种跳外层 for 写法 |
| `cmd/34-select-stop-broadcast` | **close(stop) 扇出取消**：388 worker 并行试密码，找到答案者 `sync.Once` 关 stop |
| `cmd/35-select-timeout` | **`time.After(d)` 超时**；命令行参数控制 t<3 / t≥3 |
| `cmd/36-select-send` | **send case**：素数筛上下游用 select 双向多路 |
...
**选型口诀**：**「是不变量保护就上锁；是数据 / 控制流就用 channel。」** 大型服务几乎一定 **两者混用**——热点结构内部用 mutex 维持不变量，边界 / 流水线 / 取消用 channel。下一章把 channel **多路复用** 起来：**`select`** 是绝大多数真实并发模式（取消、超时、扇入、扇出、合流）的拼装基座。

---

# Chapter 8：`select` — 多路 channel 的复用

**本章主线**：把 Ch7 的「**单路 channel**」升级为 **多路**。一条 goroutine 同时盯着多条 channel 上的 send / receive，谁先就绪走谁；多路同时就绪 **伪随机** 选一条。`select` 配 **`default`** 实现非阻塞、配 **`time.After`** 实现超时、配 **`close`** 实现扇出广播取消、配 **nil channel** 实现「**动态禁用 case**」——这五种组合几乎覆盖 channel 编程 80% 的真实模式（**取消 / 超时 / 扇入 / 扇出 / 合流**）。本章最后回答 **何时选 channel、何时选共享内存**。

**本章摘要**

| 主题 | 说明 |
|------|------|
| `select` 语义 | 等所有 case；都不就绪时阻塞；**多路就绪伪随机**；进入时 **所有** case 表达式 **求值一次** |
| `default` 分支 | 非阻塞 select：没有 case 就绪走 `default`；轮询 / 心跳常用 |
| **bare `break` 陷阱** | `break` 只跳 select！跳外层 for 用 **`return`** / **labeled `break`** / **`done` 标志** |
| send case | `case ch <- v:` 让发送也参与多路复用；上下游能发就发、能收就收 |
| close 作取消 | 扇出 worker `select { case <-stop: return; default: 干活 }`；任一处发现 → `close(stop)` 全员退出 |
| 超时 | `case <-time.After(d):` 给单次 select 加超时；高频请求改用 `time.NewTimer` 或 `context.WithTimeout` |
| nil channel 在 select | nil case **永远不就绪**——**动态启用 / 禁用** 某条路径 |
| 已关 channel 陷阱 | 已关 channel 的 `<-` 永远 ready（读零值）→ select 会 **忙转**；标准修法 **置 nil 屏蔽 case** |
| fan-in | 多源合并 = `select` 多个 receive + `for src!=nil` + 关闭即置 nil |
| fan-out + collect | 每路独立 `<-chan T` 收结果；不用锁就完成并行汇总（vs Ch4 `cmd/09` mutex 版本） |
| 范式选型 | **简洁 / 解耦优先用 channel；高频小操作 / 大数据共享优先用 memory + lock**；二者常混用 |

---

## 1. 为什么需要 `select`：从「等一条」到「等多条」

Ch7 里所有等待都是单路：`<-ch` 阻塞到某一条 channel 上。可现实里几乎一定是多路：

- 「**要么** 拿到结果，**要么** 收到取消，**要么** 超时」
- 「**任一** worker 完成都行，但要把 N 路结果合到一起」
- 「上游有就发、下游有就收」（双向多路）

如果只有 `<-ch`，要等多路只能起一堆中转 goroutine 转发到同一条 channel 上——又啰嗦又难取消。`select` 就是为这件事设计的：

```go
select {
case v := <-chA: handleA(v)
case v := <-chB: handleB(v)
case chC <- x:   // 也可以是 send case
case <-time.After(2 * time.Second):
    return errTimeout
default:         // 可选：没人就绪就走 default
    spinOrSleep()
}
```

最小可跑例子：`cmd/32-select-multiplex` 用 `select` 把两条不同节奏的源 channel 合到主循环里读出。

---

## 2. `select` 的执行语义

[Go Spec # Select_statements](https://go.dev/ref/spec#Select_statements) 把规则定得很死，几条值得记住：

1. **进入 select 时**，所有 case 里的 **channel 表达式** 与 send 的 **右值表达式** 各 **求值一次**，按 **源代码顺序**。这意味着 `case ch <- f():` 中的 `f()` **每次进入 select 都会调用一次**，不论这条 case 是否被选中——见 §7 的 prime 例子。
2. **统计可执行 case**：哪些 receive 已经有值 / 已关闭（`ok=false` 也算）、哪些 send 对方已就绪。
3. **多条可执行** → **伪随机** 选一条。**不要**写成「我总希望 A 优先」的代码。
4. **没有可执行**：
   - 无 `default` → **阻塞** 到至少一条就绪。
   - 有 `default` → **立刻** 走 `default`（非阻塞模式）。
5. 选中的 case 执行完，select 整体结束（不会自动循环——循环是外层 for 的事）。

```mermaid
flowchart TB
  enter["进入 select：求值所有 case 的 channel / 右值"] --> ready["统计可执行 case"]
  ready -->|多条| rand["伪随机选一条 → 执行"]
  ready -->|一条| pick["直接执行该条"]
  ready -->|零条| dflt{"有 default？"}
  dflt -->|有| run_default["走 default"]
  dflt -->|无| block["阻塞，直到至少一条就绪"]
  block --> ready
```

---

## 3. `default`：非阻塞 select

`default` 是 select 的「逃生口」：**当下没人就绪就先做点别的**。常见用途：

- **非阻塞收**：`select { case v := <-ch: use(v); default: }` —— 试试有没有值，没有就跳过。
- **非阻塞发**：`select { case ch <- v: ; default: dropOrLog() }` —— 队列满了直接丢弃 / 走旁路（背压策略之一）。
- **轮询 + 干别的活**：见 §5 的密码爆破 worker，每轮 select **检查一下取消信号**，没有就走 default 干一格活。

完整例：`cmd/33-select-default-nonblock`。

**注意**：把 `default` 写在「等结果」的循环里很容易写出 **CPU 自旋**。要么在 `default` 里 `time.Sleep`，要么干脆不要 `default`、让 select 直接阻塞——大多数取消 / 结果合并场景属于后者。

---

## 4. `break` 只跳 select：三种「跳外层 for」的写法

```go
for {
    select {
    case v := <-ch:
        // break 只跳出 select！外层 for 还在
        break
    }
    // 上面的 break 落到这里继续循环
}
...
这是初学者最容易写错的一处。**跳出外层 for 的三种正确写法**（任选一种，按场景）：

```go
// 1) return：能立刻返回函数就最简单
for {
    select {
    case v := <-ch: handle(v); return
    }
}

// 2) done 标志：要在循环结束后再做收尾时用
done := false
for !done {
    select {
    case v := <-ch: handle(v); done = true
    }
}
finalize()

// 3) labeled break：嵌套深、不想拉变量时用
loop:
for {
    select {
    case v := <-ch: handle(v); break loop
    }
}
finalize()
```

`cmd/33-select-default-nonblock` 主体用 `return`，文件底部注释里给出 1) 和 3) 的等价写法。

---

## 5. `close` 作扇出取消信号：一处发现 → 全员退出

Ch7 §5.4 已经讲过 **close 是一对多广播**。结合 `select`，扇出 worker 的取消模板长这样：

```go
stop := make(chan struct{})
for _, chunk := range partitions {
    go func(c Range) {
        for n := c.From; n < c.To; n++ {
            select {
            case <-stop: // 任一处 close(stop) → 这里立刻 ready
                return
            default:
                if check(n) {
                    result <- n
                    once.Do(func() { close(stop) }) // 找到答案的人负责广播
                    return
                }
            }
        }
...
`cmd/34-select-stop-broadcast` 用这个模板暴力试 `"go far"`：把 27⁶ 大小的 keyspace 切成 1M 一段，每段一个 goroutine；找到答案的 goroutine 用 `sync.Once` 调一次 `close(stop)`，全体 worker 在下一轮 select 里命中 `<-stop` 立即退出。

**关键点**：

- **只有发现答案的人** 关 channel；用 `sync.Once` 防止「凑巧多人同时找到」的双 close panic（见 Ch7 §8.1）。
- 取消传播 **与 worker 数量无关**——加 100 个 worker 还是一行 `close(stop)`。
- 这个模板换成 `context.Context` 几乎一字不改：把 `<-stop` 换成 `<-ctx.Done()`，把 `close(stop)` 换成 `cancel()`。Ch9 / 真实代码里几乎都用后者。

---

## 6. 超时：`time.After` 与 `context.WithTimeout`

给 **一次** select 加超时最直白的写法：
...
`time.After(d)` 返回 `<-chan time.Time`，**`d` 之后** 发一个时间值。完整例：`cmd/35-select-timeout`（命令行参数指定超时秒数，可看 t<3 / t≥3 两种结果）。

**要注意 `time.After` 的「定时器泄漏」**：每次调用 **创建一个新的 Timer**，过期前不会被 GC。在 **高频循环里** 反复 `case <-time.After(d):` 会堆出大量临时 timer，影响内存与调度。可选替代：

| 写法 | 适用 |
|------|------|
| `case <-time.After(d):` | **一次性** 超时，最简洁 |
| `t := time.NewTimer(d); defer t.Stop()`，再 `case <-t.C:` | 同一 select 反复用、需要可取消 timer |
| `ctx, cancel := context.WithTimeout(parent, d); defer cancel()`，再 `case <-ctx.Done():` | **跨函数 / 跨 goroutine** 传超时；首选；同一个 ctx 还能继续往下传给底层调用 |

工程代码里 `context.WithTimeout` 几乎是唯一推荐——它把超时与取消统一成一条传播链，被 `net/http`、`database/sql`、各 RPC 库直接消费。

---

## 7. `select` 也能放 send case：双向多路复用

Ch7 的形参方向区分了「只读」「只写」，但 `select` 不要求所有 case 同向：**能 send 也能 receive**。常见用途是 **生产 / 消费可被打断**：

```go
select {
case numbers <- next():     // 上游能收就推一条
case p := <-primes:         // 下游有素数就取一条
    found++
}
```

`cmd/36-select-send` 用这个模式驱动一个素数筛：主循环里要么把新随机数推进 `numbers`，要么从 `primes` 收一个素数；下游被 `range` 自动驱动，**主流程从不显式知道「该不该等」**——`select` 自己挑。

**陷阱**：进入 select 时所有 case 的 **右值表达式都会求值一次**，所以 `case numbers <- rand.Intn(N)+1:` **每轮都会调用一次** `rand.Intn`，即使本轮没选这条 send。**右值有副作用 / 计算昂贵时要小心**：把它移到 select 外面缓存到一个变量再用。

---

## 8. nil channel 在 select 里：动态禁用 case

Ch7 §8.2 已经点过：**nil channel 上收发永远阻塞**。这条性质在 select 里直接变成 **「禁用一个 case」**：

```go
var trigger <-chan T // 默认 nil
if cond { trigger = realCh }

select {
case v := <-trigger:    // trigger 是 nil 时这条永远不就绪
    handle(v)
case <-ctx.Done():
    return
}
...
在 `cond` 不满足时，整个 select 等同于「只等 `ctx.Done()`」；条件变化后再把 `trigger` 接上即可恢复。这是 select 最优雅的用法之一，下一节会用到。

---

## 9. 已关 channel + select 的「忙转」陷阱

**已关闭 channel 上的 `<-ch` 永远 ready**：立刻返回 **零值 + `ok=false`**。这在 `for range ch` 下没问题（自动退出循环），但放到多路 `select` 里就出事——**那条 case 每轮都命中**，select 会 **忙转着读零值**：

```go
// BUG：sales 关闭后，这条 case 持续命中读零值，CPU 飙到 100%
for {
    select {
    case sale := <-sales:
        use(sale)
...
**标准修法：检测到 `ok=false` 就把 channel 变量置 nil**——配合 §8 的「nil case 永不就绪」，这条路就从 select 里被自然摘掉：

```go
for sales != nil || expenses != nil {
    select {
    case s, ok := <-sales:
        if !ok { sales = nil; continue }
        use(s)
    case e, ok := <-expenses:
        if !ok { expenses = nil; continue }
        use(e)
    }
}
```

完整例：`cmd/38-select-fan-in-disable`（同时也是下一节的 fan-in 标准实现）。**注意循环条件用 `||` 而不是 `&&`**：用 `&&` 时一关就停，会丢掉另一路剩下的数据。

---

## 10. 模式：fan-in（多源合并）

把 N 条同类型 channel 合成一条 / 一个收尾循环——**fan-in**。`select` + 「关闭置 nil」是最直接的写法（见 §9）：循环条件 `for src1!=nil || src2!=nil || …`，每条源关闭就置 nil，全 nil 即所有源耗尽，循环退出。

```mermaid
flowchart LR
  s1[("sales")] --> sel
  s2[("expenses")] --> sel
  sel{{"select"}} --> consumer["主循环：合并、汇总、逐个消费"]
  closed1["sales close → sales = nil"] -.-> sel
  closed2["expenses close → expenses = nil"] -.-> sel
```

`cmd/38-select-fan-in-disable` 把销售流 / 支出流合并到一个 P&L 累加；两条流条数不同，**置 nil 的写法保证两边都被读到底**。

`fan-in` 的另一种写法是用 **一个中转 goroutine 把多路合并到单条 channel**（典型「pipeline 合流」），适合源数量动态、想把 select 隐藏到内部时用。两种写法语义等价，挑读起来更自然的那个。

---

## 11. 模式：fan-out + collect（不用锁的并行汇总）

**fan-out**：把一份大任务切成 N 份分给 N 个 worker；**collect**：把 N 份结果汇总。CSP 风格的写法是 **每个 worker 自己一条 `<-chan T` 输出**，主流程顺序 `<-` 收齐：

```go
results := make([]<-chan []int, 0, len(urls))
for _, url := range urls {
    results = append(results, countLetters(url)) // 每个 url 一个 goroutine + 一条 channel
}
total := make([]int, 26)
for _, ch := range results {
    if r, ok := <-ch; ok {
        for i, c := range r { total[i] += c }
    }
}
```

`cmd/39-channels-fan-out-collect` 跑 31 篇 RFC 文档的字母频率统计：每个 URL 起一个 goroutine 数自己那份频率，结果各自一条 channel，主流程顺序累加。

**对比 `cmd/09-rfc-freq-mutex`**（Ch4 的版本）：那里用一把 `sync.Mutex` 保护 **共享的 `freq map`**，所有 goroutine 抢锁更新；这里 **每个 worker 独占** 自己的 `[]int`，结果走 channel 传出——**没有共享可变状态，自然没有 data race**。

| 维度 | mutex 版（cmd/09）| channel 版（cmd/39）|
|------|-------------------|---------------------|
| 共享状态 | 一份 freq map + 一把 mutex | 无；每个 worker 独占自己的切片 |
| 失败处理 | 锁内异常会污染共享 map | 单条失败 close 即丢弃，不影响其它 |
| 内存 | 单份共享结构 | N 份独立 + 一次最终汇总 |
| 心智 | 共享内存：要论证「每次更新临界区正确」 | CSP：每路独立 + 顺序汇总，**几乎不用想** |

工程上没有「哪个一定更好」的结论；**channel 版可读性常常胜出**，尤其当结果结构本身就是「一份完整数据」（频率切片 / DTO / Result struct）。

---

## 12. message passing vs memory sharing：什么时候选哪个

| 维度 | channel（消息传递） | mutex / atomic（共享内存）|
|------|---------------------|---------------------------|
| **代码简洁度** | 边界清晰，stage 之间 `<-chan` / `chan<-` 自然分割 | 需要自己论证「每个临界区都正确」，更接近底层 |
| **耦合度** | 易做成 **松耦合**：每个 goroutine 只看自己的输入 / 输出 channel | 容易共享一堆字段，慢慢变成 **紧耦合的状态机** |
| **正确性证明** | 数据所有权随消息流动，**race 几乎天然没有** | 需要文字记录「哪个字段哪把锁保护」，靠 `-race` + code review 兜底 |
| **性能** | 收发 + 调度 + GC 都有成本；**消息大或 chatty 时** 会成为瓶颈 | 临界区短时单原子 / 单 mutex 通常更快；适合 **热点小数据 + 高频读改写** |
| **典型场景** | 流水线、扇入扇出、取消 / 超时、跨 goroutine 控制流 | 计数器、配置缓存、热点 map、读多写少的小结构 |

**两条经验**：

1. **「松耦合优先 channel」**：当代码要按「输入 → 处理 → 输出」的方式划清边界，channel 几乎总是更顺手；测试时把上游换成假 channel 就能 mock 整个 stage。
2. **「热点优先共享内存」**：当一段共享状态被高频读改写，且消息传递的 chatty 程度可能成为开销时，老老实实上 mutex / atomic；**channel 不是性能银弹**。

实际工程里 **几乎一定混用**：边界 / 流水线 / 取消用 channel，热点结构（计数器、限流器、连接池）内部用 mutex 维持不变量。**先按「我要表达什么」选**，不要先按「哪个看起来更高级」选。

---

## 13. 与 Ch7 / Ch3 / `context` 的衔接
